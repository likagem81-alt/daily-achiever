...
function openApp(){user=localStorage.getItem("da4_current");if(!user)return;$("loginScreen").classList.add("hidden");$("app").classList.remove("hidden");if(!load("startDate",null))save("startDate",Date.now());seedGeomorphologyResult();seedJainismResult();seedWeatheringNotes();seedWeatheringTestResult();seedAeolianNotes();timerLoad();renderAll();updateClock();setInterval(updateClock,1000)}
